...
| `启动.bat` | 启动脚本（**双击这个**） |
| `.env` | 配置文件（含 OneNet 凭证、JWT 密钥等） |

使用步骤：

| 步骤 | 操作 |
|------|------|
| 1 | 根据实际环境修改 `.env` 中的配置（OneNet 设备信息、JWT 密钥等） |
| 2 | 双击 `启动.bat` |
| 3 | 弹出命令行窗口，显示启动信息 |
| 4 | 浏览器自动打开 `http://localhost:8000` |
| 5 | 输入账号 `admin`，密码 `admin123` 登录 |
| 6 | 关闭命令行窗口即可退出程序 |
...
复制 `.env.example` 为 `.env`，然后编辑 `.env` 填入你的 OneNet 设备信息：
...
ONENET_DEVICE_NAME="你的设备名称"
ONENET_TOKEN="你的鉴权Token"
JWT_SECRET="替换为随机字符串"
```

> 这三项 OneNet 凭证从 OneNet 物联网平台控制台 → 设备详情页获取。  
> JWT_SECRET 可用 `python -c "import secrets; print(secrets.token_urlsafe(32))"` 生成随机字符串。
...
| **依赖条件** | 无额外依赖 | 需 `.env` 中启用 `LDAP_ENABLED=True` 并填写域控服务器地址 |
...
所有配置通过 `.env` 文件管理（`config.py` 自动从环境变量/`.env` 文件读取）。复制 `.env.example` 为 `.env` 后编辑即可：

```bash
# ── OneNet 物联网平台配置（必填）──
ONENET_PRODUCT_ID="你的产品ID"        # OneNet 产品 ID
ONENET_DEVICE_NAME="你的设备名称"     # OneNet 设备名称
ONENET_TOKEN="你的鉴权Token"          # OneNet API 鉴权 token
...
> 💡 `config.py` 不再需要手动编辑 — 所有配置项均从环境变量/`.env` 文件读取，优先级：环境变量 > `.env` 文件 > 默认值。
...
A: 检查 `.env` 文件中的 OneNet 凭证（ONENET_PRODUCT_ID、ONENET_DEVICE_NAME、ONENET_TOKEN）是否正确，设备是否在线。
...
A: 修改 `.env` 文件中的 `PORT` 为其他值（如 8080），重新启动。
...
A: `.env` 中 `HOST=0.0.0.0` 已默认允许局域网访问。其他电脑访问 `http://<本机IP>:8000` 即可。注意防火墙可能需要放行 8000 端口。

**Q: 如何启用 LDAP/AD 登录？**

A: 编辑 `.env` 文件，设置 `LDAP_ENABLED=True` 并填写正确的 `LDAP_SERVER`、`LDAP_BASE_DN`、`LDAP_DOMAIN`。然后重启服务。
